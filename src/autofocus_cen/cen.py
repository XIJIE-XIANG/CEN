...
class CENConfig:
    r2_list: tuple[float, ...] = tuple(np.round(np.arange(0.05, 0.31, 0.01), 4).tolist())
    r_max: float = 0.35
    k_bins: int = 256
...
def score_curve_general(y: Sequence[float], edge_ratio: float = 0.05) -> tuple[float, dict[str, float | int]]:
...
def pick_r2_gtfree(
    ...
    for r2 in sorted(curves_by_r2):
        y = normalize_01(curves_by_r2[r2])
        score, detail = score_curve_general(y, edge_ratio=edge_ratio)
...
def estimate_focus_from_blocks(
    ...
    r2, detail = pick_r2_gtfree(curves, edge_ratio=config.edge_ratio)
    curve = curves[r2]
